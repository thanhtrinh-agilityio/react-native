...
describe('SuggestInput', () => {
  it('should renders all suggestions correctly', () => {
    const { getByText } = render(
      <SuggestInput suggestions={MOCK_SUGGESTIONS} onSuggestionPress={() => { }} />
    );
  ...
  it('should calls onSuggestionPress with correct label when pressed', () => {
    ...
    const { getByText } = render(
      <SuggestInput suggestions={MOCK_SUGGESTIONS} onSuggestionPress={onSuggestionPressMock} />
    );

    const suggestion = getByText(MOCK_SUGGESTIONS[0].label);
    fireEvent.press(suggestion);

    expect(onSuggestionPressMock).toHaveBeenCalledWith(MOCK_SUGGESTIONS[0].label);
  });
});
